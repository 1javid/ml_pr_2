...
# ─────────────────────────────────────────────────────────────
# HARDCODED DISPLAY DATA  (metrics / examples – from notebook outputs)
# ─────────────────────────────────────────────────────────────
...
# Map from display model name → MODELS key
_SKLEARN_MODELS = {
    "LDA":                  "lda",
    "QDA":                  "qda",
...
if page == "🏠 Overview":
    ...
    for col, (title, body) in zip(st.columns(3), [
        ...
        ("⏱️ Delays Hurt", "Each 10-min increase in arrival delay reduces satisfaction log-odds by ~0.12."),
    ]):
        col.markdown(
...
# ─────────────────────────────────────────────────────────────
    ...
    mode_badge = (
        "🟢 **Using trained sklearn models** loaded from `models/`"
        if MODELS_LOADED else
        "🟡 **Fallback mode** — run the last notebook cell to generate `models/` for real predictions"
    )
    st.info(mode_badge)
    st.divider()
    ...
    with st.form("predictor_form"):
        ...
        with c4:
            st.markdown("**Service Ratings — Part 2 (1–5)**")
